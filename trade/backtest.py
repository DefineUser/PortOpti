...
import numpy as np
from portopti.trade import trader
from portopti.marketdata.datamatrices import DataMatrices
import logging
from portopti.tools.trade import calculate_pv_after_commission
...
class BackTest(trader.Trader):
    def __init__(self, config, net_dir=None, agent=None, agent_type="nn"):
        trader.Trader.__init__(self, 0, config, 0, net_dir,
                               initial_investment=10000, agent=agent, agent_type=agent_type)
        if agent_type == "nn":
